...
function FetchQueryContext(query, model, requestKey) {
    this.model = model;
    this.query = query;
    this.requestKey = requestKey;
}

FetchQueryContext.prototype.run = function(req, res, next, cb) {
    var self = this;
    self.model.find(self.query, function(err, objects) {
        if ( err ) {
            return cb(err);
        }

        req[self.requestKey] = objects;
        cb();
    });
};

module.exports.FetchQueryContext = FetchQueryContext;
module.exports.fetchObjectsFromQuery = function() {
    return function(ctxs) {
        if ( !(ctxs instanceof Array) ) {
            throw new Error("FetchQueryContexts must be an array.");
        }
        return function(req, res, next, pass) {
            async.each(ctxs, function(ctx, cb) {
                ctx.run(req, res, next, cb);
